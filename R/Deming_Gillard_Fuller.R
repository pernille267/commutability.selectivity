...
Deming_Gillard_Fuller <- function(data, level = 0.99, R = 3, Np = 1e3, evaluated_materials = NULL){
  ...
  s.vv <- crossprod(y - mean(y) - b1*(x - mean(x)))/(n-2) # training MSE
  var.b1 <-  ((((b1) ^ 2) / (n * sxy^2))) * (sxx*syy - sxy^2) # Miller (Modified Fuller) (a)
  var.b0 <- s.vv/n + mean(x)^2 * var.b1 # Fuller (asymptotic results)
  cov.b0.b1 <- -mean(x) * var.b1 # Fuller (asymptotic result)
  v <- rbind(c(var.b0, cov.b0.b1),c(cov.b0.b1, var.b1))
  t <- qt(p = (1-level)/2, df = n-2, lower.tail = FALSE)
  b <- c(b0,b1)
  X <- cbind(rep(1,times=length(nx)),nx)
  ny <- (X %*% b)[,]
