...
#' @param p A number signifying the average proportion of clinical samples being relocated
...
#'
#' @description Simulation of data with different selectivity profiles defined by first setting. To be used in simulations. Setting either simulation parameters to 'r' will invoke random generation of these numbers. Note that specifying parameters both outside and inside of parameter_row will make those parameters outside being overridden.
#'
#' @details Combine with \code{lapply()} or \code{sapply()} to simulate more than one data table. For parallel computing use \code{mc_sdwdnsp1()}. This function should only be used by advanced users. It is not user friendly for those not familiar with R
#'
#' @return A data table with 4 columns, two of which are measurement results from two MSs in comparison having differences in selectivity profiles defined by the first setting. In addition, n * R rows
#' @export
#'
#' @examples sdwdnsp1()
...
sdwdnsp1 <- function(n = "r", R = "r", cv_vec = "r", ci = "r", mmax = 2.5, p = 0.05, parameter_row = c(1), include_relocated = FALSE){
  ...
  dt <- setDT(dt)[,.(ReplicateID = ReplicateID,
