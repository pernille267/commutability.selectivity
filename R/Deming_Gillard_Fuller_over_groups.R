...
#' @param Np An integer, which captures the number of pointwise prediction intervals making the prediction band across the concentration range. Not relevant is evaluated_materials are !NULL
...
#' @export
#'
#' @details This is the prediction interval estimation procedure using parts from Jonathan Gillard's PhD thesis work and Wayne Fuller's book Measurement error models. This is recommended to use over Deming_CLSI_EP14_over_groups if the calculated k value is larger than 1
#'
...
Deming_Gillard_Fuller_over_groups <- function(data, groups = "Comparison", level = 0.99, R = 3, Np = 1e3, evaluated_materials = NULL, column_order = c("Comparison", "SampleID", "MP_B", "MP_A", "fit", "lwr" , "upr")){
  ...
  if(!is.null(evaluated_materials)){
    pb_at_eqmas <- mapply(FUN = function(x,y) Deming_Gillard_Fuller(data = x, evaluated_materials = y), data, evaluated_materials, SIMPLIFY = FALSE)
    ...
    rbindlist(l = lapply(X = data, FUN = Deming_Gillard_Fuller, level = level, R = R, Np = Np, evaluated_materials = NULL), idcol = paste(groups, sep = " - "))
  }
